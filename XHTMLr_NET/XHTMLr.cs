...
namespace XHTMLr
{
	public sealed class XHTML
	{
		...
		private static Dictionary<string, int> _Entities = typeof(Entities)
		...
		private static string Entity(string name)
		{
			...
			if (value == 153)
			{
				value = 8482; //convert html valid TM to an xml valid one
			}
		...
		{
			if (OpenTags.Count == 0 && !EntitiesOnly)
			{
				value = value.Trim();
			}
		...
		private string Process(string html, Options options = Options.Default)
		{
			...
			using (Output = new StringWriter())
			{
				...
				if (NumTagsWritten == 0 && !EntitiesOnly)
				{
					return string.Empty;
				}
		...
		private string ReadUntil(string marker, StringComparison comparison = StringComparison.Ordinal)
		{
			...
			using (var output = new StringWriter())
			{
				while (true)
				{
					...
					last += c[0];
					output.Write(c[0]);
					if (string.Equals(marker, last, comparison))
					{
						break;
					}
				}
		...
		private void ReadWhileWhitespace()
		{
			...
			{
				var c = Peek();
				if (c == null || !_Whitespace.Contains(c.Value))
				{
					return;
				}
