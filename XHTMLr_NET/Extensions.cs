	...
	public static class Extensions
	{
		...
		public static FormData Serialize(this XElement form)
		{
			...
			foreach (var input in inputs)
			{
				...
				if (type.Is("file"))
				{
					values.Add(name, new FilePointer { File = value });
				}
				else
				{
		...
		{
			if (int.TryParse(input, System.Globalization.NumberStyles.HexNumber, null, out int ret))
			{
				return ret;
			}
